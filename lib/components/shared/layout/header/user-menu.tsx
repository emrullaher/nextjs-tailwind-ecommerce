...
export const UserMenu = () => {
	...
	const menuRef = useClickOutside(closeUserMenu);
	...
	return (
		<>
			{!isAuthenticated && (
				<Link
					className="ml-5 block rounded-md bg-teal-600 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-teal-700"
					href="/login"
				>
					Login
				</Link>
			...
			{isAuthenticated && (
				...
				>
					<button
						...
						className="relative ml-5 text-center text-gray-700 transition dark:text-white"
					>
					...
					<div
						...
						>
							...
							/>
							<div className="mx-1">
								<h1 className="text-sm font-semibold capitalize text-gray-700 dark:text-gray-200">
									{user?.name?.firstname}
									{user?.name?.lastname}
								</h1>
						...
						<button
							type="submit"
							className="flex w-full transform items-center p-3 text-sm capitalize text-gray-600 transition-colors duration-300 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white"
							onClick={onLogout}
						>
							<svg
